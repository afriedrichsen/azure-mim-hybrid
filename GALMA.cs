	...
	public class MASynchronizer : GALSyncCs.Synchronizer, IMASynchronization
	{
		...
		public void MapAttributesForImport(string FlowRuleName, CSEntry csentry, MVEntry mventry)
		{
			...
			switch (FlowRuleName) {
                ...
                // The following two rules are temporary rules to clear out Skype attributes for contact objects on Company2 side
                // Will be removed if and when Skype federation is enabled. - AFRIEDRICHSEN 11/1/17
                case "afriedSkypeSourceSIDRule":
                    if(csentry["msRTCSIP-OriginatorSid"].IsPresent || mventry["Ms-RTCSIP-Originator-Sid"].IsPresent)
                    {
                        mventry["msRTCSIP-OriginatorSid"].Delete();
                    }
                    break;

                case "afriedSkypeSourceObjectDNRule":
                    if (csentry["msDS-SourceObjectDN"].IsPresent || mventry["Ms-DS-Source-Object-DN"].IsPresent)
                    {
                        mventry["Ms-DS-Source-Object-DN"].Delete();
                    }
                    break;
                ...
                // DominoSync create targetAddress as all lower case. We want to conform to this to prevent racing - AFRIEDRICHSEN 10/24/17
                case "afriedTargetAddressRule":
                    if(csentry["targetAddress"].IsPresent)
                    {
                        mventry["targetAddress"].Value = csentry["targetAddress"].Value.ToLower();
                    }
                    break;
                case "afriedEmailRule":
                    if (csentry["mail"].IsPresent)
                    {
                        mventry["mail"].Value = csentry["mail"].Value.ToLower();
                    }
                    break;
                ...
                case "afriedTitleRule":
                    ...
                    {
                        mventry["title"].Value = "Contractor";
                    }
                    break;
                    /* case "afriedDisplayNameRule":
                         if (csentry["adfNickname"].IsPresent)
                         {
                             ...
                             if (mailSuffix.Contains("subsidiarydomain1") || mailSuffix.Contains("subsidiarydomain2"))
                             {

                              mventry["displayName"].Value = csentry["displayName"].Value + " (Company2)";


                             }
                             else {

                                 mventry["displayName"].Value = csentry["displayName"].Value + " (Company1)";
                ...
                case "afriedDisplayNameRule":
                    if (csentry["mail"].IsPresent)
                    {
                        ...
                        if (csentry["adfNickname"].IsPresent)
                        {
                            ...
                            {
                               strInitials = csentry["initials"].Value;
                            }
                            else
                            {
                                strInitials = "";
                ...
                case "afried2DisplayNameRule":
                    ...
                    {
                        mventry["displayName"].Value = csentry["cn"].Value + " (Company2)";
                    }
                    break;
		...
		private void IAFMapiRecipient(CSEntry csentry, ref MVEntry mventry)
		{
			...
			if (csentry[HOME_MDB].IsPresent) {
                ...
                //We are changing this to true in every situation because we need Rich Text emails between Company1 and Company2.
                // We can trust the remote mail platform can handle rich text. - AFRIEDRICHSEN 12/8/17
				mventry[MAPI_RECIPIENT].BooleanValue = true;
			}
